...
if __name__ == "__main__":
  ...
  for i in xrange(0, 2 * nNodes * perNodeMem / GB):
      task = Task()
      task.start = 0
      task.type = "map"
      task.priority = 20
  ...
  job = Job()
  job.queue = 'd'
  job.start =  MIN * 1000 / 2 # job start 30 sec later
  job.user = 'wangda'
  job.amMem = minAlloc * 2 
  jobs.append(job)
  job.tasks = []
  # run task on 30% of cluster nodes
  for i in xrange(0, nNodes * perNodeMem / GB):
      task = Task()
      task.start = 0
      task.type = "map"
      task.priority = 20
  ...
  for i in range(0, len(jobs)):
    ...
    s += '"job.user" : "' + job.user + '",'
    s += '"job.id" : "' + 'job_' + str(i) + '",'
    s += '"job.am.resource.memory" : "' + str(job.amMem) + '",'
    s += '"job.tasks" : ['
